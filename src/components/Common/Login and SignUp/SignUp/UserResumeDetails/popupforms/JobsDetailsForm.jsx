...
const JobsDetailsForm = ({ closePopup, setShowPopup }) => {
  ...
  const addJob = () => {
    ...
    if (isValid) {
      ...
      dispatch(addDetail({ data, details: "Jobs" }));
      setShowPopup(false);
      reset();
      toast.success("saved");
    } else {
      ...
      <div className="absolute  left-0 right-0 lg:w-[500px]  bg-white p-10 pb-20 space-y-5 mx-auto shadow-2xl rounded-2xl">
        ...
        <div className="flex flex-col space-y-1 w-full">
          ...
          <select
            ...
            {...register("Job_Profile", { required: "select one" })}
            defaultValue=""
          >
          ...
          <p className="text-red-600  font-normal">
            {errors && errors?.Job_Profile?.message}
          </p>
        </div>
...
export default JobsDetailsForm;
